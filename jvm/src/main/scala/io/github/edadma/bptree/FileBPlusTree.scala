 ...
 *
 * @constructor creates an object to provide access to a B+ Tree contained within `file` with its tree descriptor record at offset `tree` and with a branching factor of `order`.
 ...
 */
class FileBPlusTree[K: Ordering, V](protected val file: RandomAccessFile, protected val tree: Long, val order: Int) extends BPlusTree[K, V] with FileBPlusTreeFormat:
  ...
  protected var root: Long = 0
  protected var first: Long = 0
  protected var last: Long = 0
  protected var lastlen: Int = 0
  ...
  if file.length == 0 then
    ...
    if header.toList != "B+ Tree v1  ".getBytes.toList then sys.error("bad file header")

    if file.readShort != order then sys.error("order not the same as on disk")
  ...
  protected def writeEmptyRecord(node: Long): Unit =
    file.writeLong(node)
    file.writeLong(node)
    file.writeLong(node)

  protected def addBranch(node: Long, branch: Long): Unit = setBranch(node, nodeLength(node), branch)

  protected def addKey(node: Long, key: K): Unit =
    setKey(node, nodeLength(node), key)
    nodeLength(node, nodeLength(node) + 1)

  protected def freeKey(node: N, index: Int): Unit = dispose(node + NODE_KEYS + index * DATUM_SIZE)

  protected def freeNode(node: Long): Unit = free(node, BLOCK_SIZE)

  protected def freeValue(node: N, index: Int): Unit = dispose(node + LEAF_VALUES + index * DATUM_SIZE)

  protected def getBranch(node: Long, index: Int): Long =
  ...
  protected def getBranches(node: Long): Seq[Long] =
    new AbstractSeq[Long] with IndexedSeq[Long]:
      def apply(idx: Int): Long = getBranch(node, idx)
      def length: Int = nodeLength(node) + 1

  protected def getKey(node: Long, index: Int): K =
    (if savedNode == node then readDatumArray(savedKeys, index * DATUM_SIZE)
     else readDatumFile(node + NODE_KEYS + index * DATUM_SIZE)).asInstanceOf[K]

  protected def getKeys(node: Long): Seq[K] =
    new AbstractSeq[K] with IndexedSeq[K]:
      def apply(idx: Int): K = getKey(node, idx)
      def length: Int = nodeLength(node)
  ...
  protected def getValue(node: Long, index: Int): V = readDatumFile(node + LEAF_VALUES + index * DATUM_SIZE).asInstanceOf[V]

  protected def getValues(node: Long): Seq[V] =
    new AbstractSeq[V] with IndexedSeq[V]:
      def apply(idx: Int): V = getValue(node, idx)
      def length: Int = nodeLength(node)

  protected def insertInternal(node: Long, keyIndex: Int, key: K, branchIndex: Int, branch: Long): Unit =
    val len = nodeLength(node)

    if len < order - 1 then
      nodeLength(node, len + 1)

      if keyIndex < len then copyKeys(node, keyIndex, len, node, keyIndex + 1)

      if branchIndex < len + 1 then
      ...
      if savedNode != NUL then sys.error("a node is already being saved")
      ...
      getBranches(node).view.slice(branchIndex, len + 1).copyToArray(savedBranches, branchIndex + 1)
      savedBranches(branchIndex) = branch
      savedLength = len + 1
      savedNode = node

  protected def insertLeaf(node: Long, index: Int, key: K, value: V): Unit =
    val len = nodeLength(node)

    if len < order - 1 then
      nodeLength(node, len + 1)

      if index < len then copyLeaf(node, index, len, node, index + 1)
  ...
  protected def moveInternal(src: Long, begin: Int, end: Int, dst: Long, index: Int): Unit =
    if savedNode == NUL then
      ...
      val dstKeys = savedKeys.view.slice(begin * DATUM_SIZE, end * DATUM_SIZE).toArray
      val dstBranches = savedBranches.view.slice(begin + 1, end + 1).toArray
      ...
      for i <- 0 to begin do setBranch(src, i, savedBranches(i))

      for (b, i) <- dstBranches.zipWithIndex do setBranch(dst, i + 1, b)

      nodeLength(src, len)
      nodeLength(dst, end - begin)
      savedNode = NUL

  protected def moveLeaf(src: Long, begin: Int, end: Int, dst: Long, index: Int): Unit =
    val srclen = nodeLength(src)
    val dstlen = nodeLength(dst)
    ...
    if savedNode == NUL then
      ...
      val dstKeys = savedKeys.view.slice(begin * DATUM_SIZE, end * DATUM_SIZE).toArray
      val dstValues = savedValues.view.slice(begin * DATUM_SIZE, end * DATUM_SIZE).toArray
  ...
  protected def nodeLength(node: Long): Int =
    if node == savedNode then savedLength
    else
      file.seek(node + NODE_LENGTH)
      file.readShort

  protected def nodeLength(node: Long, len: Int): Unit =
    file.seek(node + NODE_LENGTH)
    file.writeShort(len)

  protected def nul: Long = 0

  protected def removeInternal(node: Long, keyIndex: Int, branchIndex: Int): Int =
    if node == savedNode then
      ...
      if keyIndex < newlen then copyKeys(node, keyIndex + 1, len, node, keyIndex)

      if branchIndex < len then
  ...
  protected def dispose(addr: Long): Unit =
    file.seek(addr)
    ...
    file.readByte match
      ...
      case TYPE_ARRAY =>
        ...
        for i <- 0 until len do dispose(i * DATUM_SIZE + array + 4)

        free(array, len * DATUM_SIZE + 4)
      case TYPE_MAP =>
        val rec = file.readLong

        new FileBPlusTree[K, V](file, rec, order).traverseBreadthFirst(nodes =>
          val isleaf = isLeaf(nodes.head)

          for n <- nodes do
            for i <- 0 until nodeLength(n) do
              freeKey(n, i)

              if isleaf then freeValue(n, i)
  ...
  protected def removeLeaf(node: Long, index: Int): Int =
    val len = nodeLength(node)
    val newlen = len - 1

    nodeLength(node, newlen)

    if index < newlen then copyLeaf(node, index + 1, len, node, index)

    newlen

  protected def setBranch(node: Long, index: Int, branch: Long): Unit =
    file.seek(node + INTERNAL_BRANCHES + index * POINTER_SIZE)
    file.writeLong(branch)

  protected def setFirst(leaf: Long): Unit =
    file.seek(tree + TREE_FIRST_PTR)
    file.writeLong(leaf)

  protected def setKey(node: Long, index: Int, key: K): Unit = writeDatumFile(node + NODE_KEYS + index * DATUM_SIZE, key)

  protected def setLast(leaf: Long): Unit =
    file.seek(tree + TREE_LAST_PTR)
    file.writeLong(leaf)

  protected def setNext(node: Long, p: Long): Unit =
    file.seek(node + NODE_NEXT_PTR)
    file.writeLong(p)

  protected def setParent(node: Long, p: Long): Unit =
    file.seek(node + NODE_PARENT_PTR)
    file.writeLong(p)

  protected def setPrev(node: Long, p: Long): Unit =
    file.seek(node + NODE_PREV_PTR)
    file.writeLong(p)

  protected def setRoot(node: Long): Unit =
    file.seek(tree + TREE_ROOT_PTR)
    file.writeLong(node)

  protected def setValue(node: Long, index: Int, v: V): Unit = writeDatumFile(node + LEAF_VALUES + index * DATUM_SIZE, v)


  def close: Unit = file.close()


  protected def copyKeys(src: Long, begin: Int, end: Int, dst: Long, index: Int): Array[Byte] =
  ...
  protected def copyLeaf(src: Long, begin: Int, end: Int, dst: Long, index: Int): Unit =
  ...
  protected def copyInternal(src: Long, begin: Int, end: Int, dst: Long, index: Int): Unit =
  ...
  protected def decode(in: DataInput): Any =
    def readUTF8(s: DataInput, len: Int): String =
    ...
    in.readByte match
      ...
      case TYPE_ARRAY =>
        ...
        new scala.collection.immutable.IndexedSeq[Any]:
          ...
          override def toString: String = mkString("Array(", ", ", ")")
      case TYPE_MAP =>
  ...
  protected def readDatumArray(array: Array[Byte], index: Int): Any =
  ...
  protected def writeDatumArray(array: Array[Byte], index: Int, datum: Any): Unit =
    val os = new ByteArrayOutputStream

    encode(new DataOutputStream(os), datum)
    os.toByteArray.copyToArray(array, index * DATUM_SIZE)

  protected def writeDatumFile(addr: Long, datum: Any): Unit =
    file.seek(addr)
    encode(file, datum)

  protected def encode(out: DataOutput, datum: Any): Unit =
    datum match
      ...
      case d: String =>
        ...
        if utf.length > 8 then
          ...
          out.write(utf)
      case m: collection.Map[K @unchecked, V @unchecked] =>
        out.write(TYPE_MAP)
      ...
      case a: Seq[Any] =>
        ...
        for (e, i) <- a.zipWithIndex do writeDatumFile(array + 4 + i * DATUM_SIZE, e)

        file.seek(here)
        out.writeLong(array)
      case _ => sys.error("type not supported: " + datum)

  protected def alloc(size: Int): Long =
    file.seek(FILE_FREE_PTR)

    val ptr = file.readLong

    if ptr == NUL || size > BLOCK_SIZE then
      val addr = file.length
      val blocks = size / BLOCK_SIZE + (if size % BLOCK_SIZE == 0 then 0 else 1)
  ...
  protected def free(start: Long, size: Int): Unit =
    val blocks = size / BLOCK_SIZE + (if size % BLOCK_SIZE == 0 then 0 else 1)

    for i <- 0 until blocks do
  ...
  override protected def str(n: Long): String =
    if n == savedNode then savedKeys.mkString(if isLeaf(n) then "leaf[" else "internal[", ", ", "]")
    else super.str(n)

  private[bptree] def hex(n: Long*): Unit = println(n.map(a => "%h".format(a)).mkString("(", ", ", ")"))

  private[bptree] def dump: Unit =
    val cur = file.getFilePointer
    val width = 16

    file.seek(0)

    def printByte(b: Int): Unit = print("%02x ".format(b & 0xff).toUpperCase)

    def printChar(c: Int): Unit = print(if ' ' <= c && c <= '~' then c.asInstanceOf[Char] else '.')

    for line <- 0L until file.length by width do
      print("%10x  ".format(line).toUpperCase)

      val mark = file.getFilePointer

      for i <- line until ((line + width) min file.length) do
        if i % 16 == 8 then print(' ')
        printByte(file.readByte)

      val bytes = (file.getFilePointer - mark).asInstanceOf[Int]

      print(" " * ((width - bytes) * 3 + 1 + (if bytes < 9 then 1 else 0)))

      file.seek(mark)

      for i <- line until ((line + width) min file.length) do printChar(file.readByte.asInstanceOf[Int])
