 ...
 */
class MemoryBPlusTree[K: Ordering, V](val order: Int) extends BPlusTree[K, V]:
  ...
  protected def getParent(node: Node[K, V]): Node[K, V] = node.parent
  ...
  protected def insertLeaf(n: Node[K, V], index: Int, key: K, value: V): Unit =
  ...
  protected def newInternal(parent: Node[K, V]): Node[K, V] = new InternalNode(parent.asInstanceOf[InternalNode[K, V]])

  protected def newLeaf(parent: Node[K, V]): Node[K, V] = new LeafNode(parent.asInstanceOf[InternalNode[K, V]])

  protected def newRoot(branch: Node[K, V]): Node[K, V] =
    val res = new InternalNode[K, V](null)
    res.branches += branch
    res

  protected def nul: Node[K, V] = null
  ...
  protected def setValue(node: Node[K, V], index: Int, v: V): Unit = node.asLeaf.values(index) = v
