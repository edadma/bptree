...
class InsertionTests extends AnyFreeSpec with Matchers:
  ...
  "insert returns false for new key" in {
    val t = new MemoryBPlusTree[String, Int](3)
    t.insert("a", 1) shouldBe false
    t.insert("b", 2) shouldBe false
    t.insert("c", 3) shouldBe false
  }

  "insert returns true for existing key" in {
    val t = new MemoryBPlusTree[String, Int](3)
    t.insert("a", 1) shouldBe false
    t.insert("a", 2) shouldBe true
    t.insert("a", 3) shouldBe true
  }

  "insert overwrites value for existing key" in {
    val t = new MemoryBPlusTree[String, Int](3)
    t.insert("a", 1)
    t.insert("b", 2)
    t.insert("c", 3)

    t.search("b") shouldBe Some(2)
    t.insert("b", 99)
    t.search("b") shouldBe Some(99)
    t.wellConstructed shouldBe "true"
    t.keysIterator.toList shouldBe List("a", "b", "c")
  }

  "isEmpty reflects tree state" in {
